# nephrology-wiki — 腎專考試複習專區 (Nephro Board Exam Review Center)
...
| **2. wiki** | `/wiki/` | M2M English | CC agents | 跨 topic 整合、因果框架、EBM 細節。CC 讀取、不給人直接讀。 |
| **3. cme** | `/cme/` | zh-TW | 考生 | TSN 5-選項格式題庫、每章末題目、自製題。|

Slides 走另一 repo `copper0722/nephro-board-review`（general textbook slideshare，非考試專屬）— nephrology-wiki 只做 note/wiki/cme，有需要 slide 就 cross-link。
...
4. **Vault canonical wiki 投影** → `/wiki/`
   - 由 vault `proj/wiki/` filter→ exam-relevant topics → export 到此 repo
